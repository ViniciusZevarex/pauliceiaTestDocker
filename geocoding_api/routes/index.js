...
router.get("/", function (req, res, next) {
  return res.json({
    message: "Welcome to the Pauliceia Geocoding API!",
  });
});
...
+--------------------------------------------------*/
router.get(
  "/geolocation/:textpoint,:number,:year/json",
      ...
      if (!error) {
        ...
        if (places_filter.length == 0) {
          ...
          results.push({
            ...
            alertMsg:
              "Não encontramos pontos nesse logradouro referentes ao ano buscado (" +
              textpoint +
              ", " +
              number +
              ", " +
        ...
        places_filter = places_filter.filter(
          (el) => el.place_firstyear <= year
        );
        ...
        if (places_filter.length == 1) {
          ...
          if (
            parseFloat(places_filter[places_filter.length - 1].place_number) <
            number
          ) {
            places_filter = places.filter((el) => el.street_name == textpoint);
            places_filter = places_filter.filter(
              (el) =>
                el.place_number ==
                parseInt(places_filter[places_filter.length - 1].place_number)
            );

            //Check if only one result was found
            if (
              places_filter.length == 1 &&
              places_filter[0].place_number > 0
            ) {
              //Organize the Json results
              results.push({
          ...
          if (year > 1931) {
            ...
            request(url, function (error, response, body) {
              if (!error) {
                ...
                var streets_filter = streets.filter(
                  (el) => el.street_name == textpoint
                );
                ...
                //filter the p1
                p1 = p1.filter(
                  (el) => el.place_number == Math.max.apply(Math, numbers_p1)
                );
                ...
                //filter the p2
                p2 = p2.filter(
                  (el) => el.place_number == Math.min.apply(Math, numbers_p2)
                );
                ...
                if (p2[0].place_geom == p1[0].place_geom) {
                  //Filter the json places to get the p2
                  let new_p2 = places_filter.filter(
                    (el) => el.place_number > number
                  );
                ...
                if (endfraction > startfraction) {
                  ...
                  var sublinestring = Create.lineSubstring(
                    linemerge,
                    startfraction,
                    endfraction
                  );
                ...
                if (sublinestring == ",") {
                  ...
                  var geometry =
                    "MULTILINESTRING((" + p1_geom + "," + p2_geom + "))";
                } else {
                  if (!sublinestring) {
                    //build the street geom
                    var geometry =
                ...
                results.push({
                  ...
                  geom:
                    ...
                    ")",
                  confidence: Calculate.confidenceRateCode(
                    p1_g.split(" "),
                    p2_g.split(" "),
                    year
                  ),
